...
;Q8
(define (square x) (* x x))
(begin (square 8) (begin (/ 12 4) (square (* 2 3))))
; expect 36
(begin (/ 1 0) (* 2 3))
; expect Error
...
; expect (lambda (x) (/ x 2))
(lambda (x y) (+ x x))
; expect (lambda (x y) (+ x x))

;Q10
(define (make-zero x) (- x x))
; expect make-zero
make-zero
;expect (lambda (x) (- x x))
...
(define y 6)
(define z 7)
(define po (mu (x) (* x y z)))
(define outside (lambda (x y z) (po x)))
(outside 2 3 4)
; expect 24

(define special (mu (x) (+ x y)))
(define (out x y)
  (define (in x y)
    (special x) 
    )
...
(define (multiply-three-squares x y z)
  (* (square x) (square y) (square z)))
(multiply-three-squares (/ 4 2) (+ 6 3) (- 7 3))
; expect 5184
...
(define (can-triangle-form x y z)
  (cond 
    ((< (+ x y) z)
      False)
    ((< (+ x z) y)
...
(can-triangle-form 5 8 12)
;expect True

(can-triangle-form 3 4 9)
;expect False
;;; 1.1.7
...
(sum-of-squares (sqrt 25) (sqrt (+ (sqrt 256) (sqrt 81))))
; expect 50.0004783927034
