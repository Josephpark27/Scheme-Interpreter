...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        elif self.parent == None and symbol not in self.bindings:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        apply_operands = lambda operands: scheme_eval(operands, env)
        return scheme_apply(self, operands.map(apply_operands), env)
        # END PROBLEM 5
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        return (complete_eval(applied), env)
        # END Problem 21
...
def make_let_frame(bindings, env):
    ...
    return Frame.make_child_frame(env, symbols, values)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        Frame.define(env, target.first, MacroProcedure(target.second, expressions.second, env))
        return target.first
        # END PROBLEM 10
    else:
        bad_target = target.first if isinstance(target, Pair) else target
